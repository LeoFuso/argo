package io.github.leofuso.columba.compiler.cli.configurer

import io.github.leofuso.columba.compiler.cli.CompileCommandRunner
import io.github.leofuso.columba.compiler.cli.enableDecimalLogicalType
import io.github.leofuso.columba.compiler.cli.getAdditionalVelocityTools
import io.github.leofuso.columba.compiler.cli.getCharacterEncoding
import io.github.leofuso.columba.compiler.cli.getConverters
import io.github.leofuso.columba.compiler.cli.getStringType
import io.github.leofuso.columba.compiler.cli.getTemplateDirectory
import org.apache.avro.LogicalTypes
import org.apache.avro.Schema
import org.apache.avro.compiler.specific.SpecificCompiler
import java.io.File

class SpecificCompilerConfigurer(private val runner: CompileCommandRunner) {

    private val logger = runner.logger

    init {
        runner.additionalLogicalTypeFactories.forEach {
    ...
    fun configure(compiler: SpecificCompiler) {
        ...
        runner.additionalVelocityTools
            .map {
        ...
        runner.additionalConverters.map { Class.forName(it) }.forEach(compiler::addCustomConversion)

        runner.velocityTemplate?.let {
        ...
        runner.outputEncoding.let { compiler.setOutputCharacterEncoding(it) }
        runner.fields.stringType.let { compiler.setStringType(it) }
        runner.fields.visibility.let { compiler.setFieldVisibility(it) }
        runner.fields.useDecimalType.let { compiler.setEnableDecimalLogicalType(it) }
        runner.accessors.allowSetters.let { compiler.isCreateSetters = it }
        runner.accessors.useOptionalGetters.let { compiler.isGettersReturnOptional = it }
        runner.accessors.useOptionalGettersForNullableFieldsOnly.let { compiler.isOptionalGettersForNullableFieldsOnly = it }
        runner.accessors.addExtraOptionalGetters.let { compiler.isCreateOptionalGetters = it }
    }
